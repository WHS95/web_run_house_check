...
// ─── 메인 컴포넌트 ───
const GradesTab = memo(function GradesTab({ crewId }: GradesTabProps) {
  ...
  return (
    <div className='space-y-4'>
      ...
      />
      <FadeIn key={subTab}>
        {subTab === "settings" && <GradeSettings crewId={crewId} />}
        {subTab === "recommendations" && (
          <GradeRecommendations crewId={crewId} />
        )}
        {subTab === "assignment" && <GradeAssignment crewId={crewId} />}
      </FadeIn>
    </div>
  );
});
...
const GradeSettings = memo(function GradeSettings({
  ...
  const [editGrade, setEditGrade] = useState<CrewGrade | null>(null);
  ...
  const [deleteTarget, setDeleteTarget] = useState<CrewGrade | null>(null);
  ...
  const fetchGrades = useCallback(async () => {
    try {
      ...
      if (result.success) {
        const data = (result.data || []) as unknown as CrewGrade[];
        setGrades(data);
        if (data.length > 0) {
          setPeriodType(data[0].promotion_period_type || "cumulative");
        }
      }
    } catch {
      // 무시
  ...
  const handleAdd = useCallback(async () => {
    ...
    try {
      const result = await createCrewGradeAction({
        ...
        minAttendanceCount: parseInt(form.minAttendance) || 0,
        minHostingCount: parseInt(form.minHosting) || 0,
  ...
  if (loading) {
    return (
      <div className='space-y-3'>
        {[1, 2, 3].map((i) => (
          <div key={i} className='h-24 rounded-xl bg-rh-bg-surface' />
        ))}
      </div>
    );
  }
  ...
  return (
    <div className='space-y-4'>
      ...
      <div className='flex items-center justify-between'>
        <span className='text-sm font-semibold text-white'>승급 기준 기간</span>
        <AdminSelect
          value={periodType}
    ...
    >
      <div className='flex items-center gap-3'>
        ...
        <div className='flex items-center gap-2 flex-1'>
          ...
          {grade.can_host && (
            <AdminBadge variant='outline'>호스트 가능</AdminBadge>
          )}
        </div>
        <button
          onClick={(e) => {
      ...
      <div className='flex gap-6 pl-11'>
        <div>
          <p className='text-[10px] text-rh-text-tertiary'>최소 참여</p>
          <p className='text-sm font-bold text-white'>
            {grade.min_attendance_count}회
          </p>
        </div>
        <div>
          <p className='text-[10px] text-rh-text-tertiary'>최소 개설</p>
          <p className='text-sm font-bold text-white'>
...
// ─── 추진 확인 서브탭 ───
const GradeRecommendations = memo(function GradeRecommendations({
  crewId,
}: {
  crewId: string;
}) {
  ...
  const handleApprove = useCallback(
    async (rec: Recommendation) => {
      haptic.medium();
      try {
        const result = await approveGradeRecommendationAction({
          crewId,
          userId: rec.user_id,
          newGradeId: rec.recommended_grade_id,
        });
        if (result.success) {
          haptic.success();
          setRecs((prev) => prev.filter((r) => r.user_id !== rec.user_id));
        }
      } catch {
        haptic.error();
      }
  ...
  const handleDeny = useCallback(
    async (rec: Recommendation) => {
      haptic.medium();
      try {
        const res = await fetch("/api/admin/grade-recommendations/deny", {
          method: "POST",
          headers: {
  ...
  const handleApproveAll = useCallback(async () => {
    haptic.medium();
    try {
      const result = await approveAllGradeRecommendationsAction({ crewId });
      if (result.success) {
        haptic.success();
        setRecs([]);
      }
  ...
  if (loading) {
    return (
      <div className='space-y-3'>
        {[1, 2].map((i) => (
          <div key={i} className='h-32 rounded-xl bg-rh-bg-surface' />
        ))}
      </div>
    );
  }
              ...
              <div className='px-4 py-4 rounded-xl bg-rh-bg-surface space-y-3'>
                ...
                <div className='flex items-center gap-2'>
                  ...
                  <div className='flex items-center gap-1'>
                    ...
                    <span className='text-xs text-rh-text-muted'>→</span>
...
const GradeAssignment = memo(function GradeAssignment({
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (mResult.success) {
          setMembers((mResult.data as unknown as Member[]) || []);
        }
        if (gResult.success) {
          setGrades((gResult.data || []) as unknown as CrewGrade[]);
        }
      } catch {
        // 무시
      } finally {
  ...
  if (loading) {
    return (
      <div className='space-y-3'>
        {[1, 2, 3].map((i) => (
          <div key={i} className='h-16 rounded-xl bg-rh-bg-surface' />
        ))}
      </div>
    );
  }
          ...
          <AnimatedItem key={m.id}>
            <div className='flex items-center justify-between px-4 py-3 rounded-xl bg-rh-bg-surface'>
              <div className='flex items-center gap-3'>
                ...
                <div>
                  <div className='flex items-center gap-2'>
                    ...
                    <AdminBadge variant={m.grade_override ? "accent" : "muted"}>
                      {getGradeName(m.crew_grade_id)}
                    </AdminBadge>
                  </div>
                  {m.grade_override && (
                    <button
              ...
              <div className='shrink-0'>
                <select
                  value={m.crew_grade_id || ""}
                  onChange={(e) => {
                    const val = e.target.value;
                    if (val) {
                      handleAssign(m.id, parseInt(val));
                    }
                  }}
