...
const NoticeWriteForm = memo(function NoticeWriteForm() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      alert(e instanceof Error ? e.message : "공지 등록에 실패했습니다.");
    } finally {
      setSubmitting(false);
    }
  ...
  const handleConfirmPush = useCallback(async () => {
    ...
    try {
      ...
      if (!result?.success) {
        alert(
          result?.message ?? "푸시 발송에 실패했습니다. 공지는 등록되었습니다.",
        );
      }
    } catch (e) {
      console.error("[notice write] push failed:", e);
      alert("푸시 발송에 실패했습니다. 공지는 등록되었습니다.");
    } finally {
      setPushing(false);
    ...
    <FadeIn>
      <div className='flex flex-col flex-1'>
        <div className='flex-1 px-4 pt-5 space-y-6'>
          ...
          <div className='space-y-2'>
            <span className='text-[14px] text-white font-medium'>카테고리</span>
          ...
          {/* 내용 */}
          <div className='space-y-2'>
            <span className='text-[14px] text-white font-medium'>내용</span>
            <textarea
      ...
      >
        <p className='text-[14px] text-rh-text-secondary leading-relaxed'>
          공지가 정상적으로 등록되었습니다.
          <br />
          확인 버튼을 누르면 크루원 전체에게 푸시 알림이 발송됩니다.
        </p>
      </AdminModal>
    </FadeIn>
  );
