...
/* ── 상수 ── */
const CLS_CARD = "rounded-xl bg-rh-bg-surface px-4 py-3";
const CLS_BTN_PRIMARY =
  "w-full h-12 rounded-xl bg-rh-accent" +
  " text-[15px] font-semibold text-white";
const CLS_SUB_TEXT = "text-xs text-rh-text-secondary";
...
const LocationTab = memo(function LocationTab({
  ...
  const [isEnabled, setIsEnabled] = useState(locationBasedAttendance);
  const [allowUnregistered, setAllowUnregistered] = useState(
    allowUnregisteredLocation,
  );
  const [accuracyRange, setAccuracyRange] = useState(initialAccuracyRange);
  const [savingRange, setSavingRange] = useState(false);
  const [viewMode, setViewMode] = useState<"list" | "form">("list");
  const [editingLocation, setEditingLocation] = useState<CrewLocation | null>(
    null,
  );
  ...
  const handleUnregisteredToggle = useCallback(
    async (enabled: boolean) => {
      haptic.medium();
      try {
        const result = await updateAllowUnregisteredLocationAction({
          crewId,
          allow: enabled,
        });
        if (result.success) {
  ...
  const handleSave = useCallback(async () => {
    ...
    if (!formData.name.trim() || isNaN(lat) || isNaN(lng)) {
      return;
    }
    ...
    try {
      ...
      if (result.success && result.data) {
        if (isEditing) {
          actions.updateLocation(result.data as unknown as CrewLocation);
        } else {
          actions.addLocation(result.data as unknown as CrewLocation);
        }
        haptic.success();
        resetForm();
      } else {
  ...
  }, [formData, editingLocation, crewId, actions, resetForm]);
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        actions.deleteLocation(editingLocation.id);
        haptic.success();
  ...
  const rangeSlider = isEnabled ? (
    <div className={CLS_CARD + " space-y-3"}>
      <div className={"flex items-center" + " justify-between"}>
        <span className='text-sm text-white'>허용 범위</span>
        <span className={"text-sm font-semibold" + " text-rh-accent"}>
          {accuracyRange}m
          {savingRange && (
            <span className={"ml-1 text-xs" + " text-rh-text" + "-secondary"}>
              저장 중...
            </span>
          )}
        </span>
      ...
      <div className='flex justify-between'>
        <span className={CLS_SUB_TEXT}>50m</span>
        <span className={CLS_SUB_TEXT}>500m</span>
      </div>
    </div>
  ) : null;
      ...
      <div className='space-y-4'>
        ...
        {isEnabled && (
          <AdminSwitchRow
            label='미등록 장소 출석 허용'
            description={
              "등록되지 않은 장소에서도" + " 출석을 진행할 수" + " 있습니다"
            }
            checked={allowUnregistered}
            onCheckedChange={handleUnregisteredToggle}
          />
        )}
        ...
        <div className='rounded-xl bg-rh-bg-surface p-3 space-y-3'>
          ...
          <div className='rounded-xl bg-rh-bg-primary p-3 space-y-2.5'>
            <p className='text-[13px] font-medium text-slate-300'>
              지도에서 위치를 선택하면 위도/경도가 자동으로 채워집니다
            </p>
            ...
            >
              <MapPin size={20} color='#8BB5F5' />
              {pickerInitial ? (
                <div className='text-center'>
                  <p className='text-sm font-medium text-white'>선택된 위치</p>
                  <p className='text-xs text-rh-text-secondary mt-0.5'>
                    {pickerInitial.lat.toFixed(6)}
                    {", "}
                    {pickerInitial.lng.toFixed(6)}
                  </p>
                </div>
              ) : (
        ...
        <div className='flex gap-2'>
          ...
          >
            {saving ? "저장 중..." : editingLocation ? "수정" : "저장"}
          </button>
        </div>
  ...
  return (
    <div className='space-y-4'>
      ...
      <div className='flex items-center gap-2'>
        <h3 className='text-sm font-semibold text-white'>등록된 장소</h3>
        <AdminBadge variant='muted'>{locations.length}</AdminBadge>
      </div>
      ...
      <button onClick={startAdd} className={CLS_BTN_PRIMARY}>
        + 새 장소 추가
      </button>
    </div>
  );
