...
const ActivityStats: React.FC<ActivityStatsProps> = ({
    ...
    return (
      <div className='space-y-[2vh]'>
        {Array.from({ length: 3 }).map((_, index) => (
          ...
          >
            <div className='h-[1.5rem] bg-rh-bg-surface rounded mb-[2vh]'></div>
  ...
  if (error) {
    return (
      <div className='flex flex-col items-center justify-center py-[8vh] text-center'>
        ...
        <p className='text-rh-text-secondary text-[0.875rem] mb-[2vh]'>{error}</p>
        <button
          onClick={loadStats}
  ...
  if (stats.length === 0) {
    return (
      <div className='flex flex-col items-center justify-center py-[8vh] text-center'>
        <div className='w-[4rem] h-[4rem] bg-rh-bg-muted/20 rounded-full flex items-center justify-center mb-[2vh]'>
      ...
      {stats.map((stat, index) => (
        <div key={index} className='bg-rh-bg-surface rounded-[1rem] p-[4vw]'>
          <div className='flex items-center justify-between mb-[2vh]'>
            ...
            <div
              className={`px-[2vw] py-[0.5vh] rounded-full text-[0.75rem] font-medium flex items-center ${getRankBadgeColor(
                stat.user_rank
              )}`}
            >
          ...
          {/* 활동 점수 바 */}
          <div>
            <div className='flex items-center justify-between text-[0.75rem] text-rh-text-secondary mb-[1vh]'>
              <span>활동 점수</span>
              <span>
                {stat.attendance_count + stat.meetings_created_count}점
              </span>
            </div>
            <div className='w-full bg-rh-bg-surface rounded-full h-[0.5vh]'>
              <div
