...
function getDisplayName(m: CrewMember): string {
  ...
  if (m.birth_year) {
    const yearSuffix = String(m.birth_year).slice(-2);
...
const MembersTab = memo(function MembersTab({ crewId }: MembersTabProps) {
  const [members, setMembers] = useState<CrewMember[]>([]);
  const [search, setSearch] = useState("");
  const [loading, setLoading] = useState(true);
  const [actionTarget, setActionTarget] = useState<CrewMember | null>(null);
  ...
  const handleRoleChange = useCallback(async () => {
    ...
    try {
      const isAdmin = actionType === "promote";
        ...
        setMembers((prev) =>
          prev.map((m) =>
            m.id === actionTarget.id
              ? {
                  ...m,
                  crew_role: isAdmin ? "CREW_MANAGER" : "MEMBER",
                }
              : m,
          ),
        );
  ...
  const admins = useMemo(
    () =>
      members.filter(
        (m) => m.crew_role === "OWNER" || m.crew_role === "CREW_MANAGER",
      ),
    [members],
  );
  ...
  const regularMembers = useMemo(
    () => members.filter((m) => m.crew_role === "MEMBER"),
    [members],
  );
  ...
  return (
    <div className='space-y-4'>
      ...
      <div className='space-y-3'>
        ...
        >
          <div className={"flex items-baseline gap-2"}>
            <h3 className={"text-sm font-semibold" + " text-white"}>운영진</h3>
              ...
              <AnimatedItem key={m.id}>
                <div
                  ...
                  className={
                    ...
                    " rounded-xl" +
                    " bg-rh-bg-surface" +
                    (m.crew_role !== "OWNER" ? " cursor-pointer" : "")
                  }
                >
      ...
      <div className='space-y-3'>
        ...
        >
          <div className={"flex items-baseline gap-2"}>
            <h3 className={"text-sm font-semibold" + " text-white"}>멤버</h3>
            <span className={"text-xs" + " text-rh-text-secondary"}>
          ...
          <>
            ...
            <AnimatedList className='space-y-2'>
              ...
              {filteredMembers.length === 0 && (
                <p
                  className={
                    "py-8 text-center" + " text-sm" + " text-rh-text-secondary"
                  }
                >
                  {search ? "검색 결과가 없습니다" : "일반 멤버가 없습니다"}
                </p>
              )}
            </AnimatedList>
          </>
      ...
      <AdminAlertDialog
        ...
        confirmLabel={actionType === "promote" ? "등록" : "해제"}
        confirmVariant={actionType === "demote" ? "danger" : "primary"}
      />
    </div>
  );
});
