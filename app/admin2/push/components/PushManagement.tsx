...
const PushManagement = memo(function PushManagement({
    ...
    getAdminCrewUsersAction({ crewId })
      .then((result) => {
        ...
        if (result.success && Array.isArray(result.data)) {
          const list: Member[] = result.data.map((u: any) => ({
            id: u.id,
            name: u.first_name,
          }));
          setMembers(list);
  ...
  const targetIds = useMemo(
    () => (mode === "all" ? members.map((m) => m.id) : Array.from(selectedIds)),
    ...
    try {
      ...
      if (result.success && result.data) {
        if (result.data.history) {
          setHistory((prev) =>
            [result.data!.history as PushHistoryRow, ...prev].slice(0, 5),
          );
        }
        ...
        alert(result.message || result.error || "발송에 실패했습니다.");
      }
    } catch {
        ...
        {mode === "select" && (
          <div className='flex flex-col gap-2 mt-2'>
            ...
            >
              <span
                className={
                  selectedIds.size > 0 ? "text-white" : "text-rh-text-muted"
                }
              >
                {selectedIds.size > 0
              ...
              <ChevronDown size={18} className='text-rh-text-tertiary' />
            </button>
