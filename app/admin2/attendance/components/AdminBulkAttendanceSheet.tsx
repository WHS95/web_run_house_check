...
function getNowKSTTime(): string {
  ...
  const mmRounded = (Math.floor(mm / 10) * 10).toString().padStart(2, "0");
  return `${hh}:${mmRounded}`;
}
  ...
  return (
    <div
      className={`flex items-center gap-1 h-7 px-2 rounded-md ${
        active ? "bg-rh-bg-primary" : "bg-rh-bg-muted/30"
      }`}
    >
      <Icon
        size={12}
        className={active ? "text-rh-text-secondary" : "text-rh-text-muted"}
      />
      <span
        className={`text-[11px] font-medium ${
          active ? "text-rh-text-secondary" : "text-rh-text-muted"
        }`}
      >
        {label}
      </span>
...
const MemberRow = memo(function MemberRow({
  ...
  return (
    <div className='flex items-center gap-3 h-11 px-3 rounded-[10px] bg-rh-bg-surface'>
      <AdminCheckbox
        checked={selected}
        onCheckedChange={() => {
          haptic.light();
      ...
      >
        <span className='text-sm font-medium text-white truncate'>
          {name}
          {birthSuffix && (
            <span className='text-rh-text-secondary'>({birthSuffix})</span>
          )}
        </span>
      </button>
      {selected && (
...
function AdminBulkAttendanceSheet({
  ...
  /* 필수 필드 완료 여부 */
  const allFieldsReady = !!date && !!time && !!locationId;
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (cancelled) return;
        if (uResult.success && Array.isArray(uResult.data)) {
  ...
  const exerciseTypeName = useMemo(
    () =>
      exerciseTypes.find((e) => String(e.id) === exerciseTypeId)?.name || "",
  ...
  const filteredUsers = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      list = list.filter((u) => u.first_name.toLowerCase().includes(q));
    }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setGuideMessage(""), 2500);
    return () => clearTimeout(timer);
  }, [guideMessage]);
  ...
  const handlePreSubmit = useCallback(() => {
    if (!allFieldsReady) {
      haptic.error();
      setGuideMessage("날짜, 시간, 장소를 모두 선택해주세요.");
      setFormExpanded(true);
      return;
    }
    if (selected.size === 0) {
      haptic.error();
      setGuideMessage("출석 처리할 회원을 선택해주세요.");
      return;
    }
    haptic.medium();
    setConfirmOpen(true);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const result = await createBulkAttendanceAction(payload);
      if (result.success) {
        haptic.success();
        onSuccess();
      } else {
        haptic.error();
        setGuideMessage(result.message || "출석 처리 중 오류가 발생했습니다.");
      }
    } catch {
      haptic.error();
      setGuideMessage("출석 처리 중 오류가 발생했습니다.");
    } finally {
    ...
    <>
      <div className='fixed inset-0 z-[60] flex flex-col bg-rh-bg-primary'>
        ...
        <div className='shrink-0 flex items-center justify-between h-14 px-4 bg-rh-bg-surface'>
          ...
          >
            ...
            <h2 className='text-lg font-semibold text-white'>일괄 출석 등록</h2>
          </button>
          <div className='w-8' />
        </div>
        ...
        >
          ...
          <div className='flex items-center gap-2 flex-wrap'>
            <InfoChip
              icon={Calendar}
              label={date ? formatDateChip(date) : "날짜"}
              active={!!date}
            />
            <InfoChip icon={Clock} label={time || "시간"} active={!!time} />
            <InfoChip
              icon={MapPin}
              label={locationName || "장소"}
              active={!!locationId}
            />
            <InfoChip
              icon={Zap}
              label={exerciseTypeName || "종류"}
            ...
            <div className='space-y-3 pb-1'>
              ...
              <div className='flex flex-col gap-1.5'>
                ...
                <div className='relative'>
                  ...
                  <div className='absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none'>
                    <Calendar size={18} className='text-rh-text-muted' />
                  </div>
                </div>
              </div>
              ...
              <div className='flex flex-col gap-1.5'>
                ...
                <div className='relative'>
                  ...
                  <div className='absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none'>
                    <Clock size={18} className='text-rh-text-muted' />
                  </div>
                </div>
              </div>
              ...
              <div className='flex flex-col gap-1.5'>
                ...
                <div className='relative'>
                  ...
                  <div className='absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none'>
                    <MapPin size={18} className='text-rh-text-muted' />
                  </div>
                </div>
              </div>
                ...
                {isLoading ? (
                  ...
                  <div className='flex gap-2 flex-wrap'>
                    {exerciseTypes.map((et) => {
                      const isActive = exerciseTypeId === String(et.id);
                      return (
                        <button
                          key={et.id}
                          type='button'
                          onClick={() => {
                            haptic.light();
                            setExerciseTypeId(String(et.id));
                          }}
          ...
          {/* 검색바 — .pen YmfNw */}
          <div className='flex items-center gap-2 h-10 px-3 rounded-lg bg-rh-bg-surface border border-rh-border'>
            ...
            {searchQuery && (
              <button
                ...
                className='shrink-0'
              >
                <X size={16} className='text-rh-text-muted' />
              </button>
            )}
          </div>
          ...
          {/* 전체 선택 — .pen lsmVj */}
          <div className='flex items-center justify-between h-10 px-3 rounded-lg bg-rh-bg-surface'>
            ...
            {isLoading ? (
              ...
              filteredUsers.map((u) => (
                <MemberRow
                  ...
                  userId={u.id}
                  name={u.first_name}
                  birthSuffix={getBirthSuffix(u.birth_year)}
        ...
        {/* Bottom CTA */}
        <div className='shrink-0 px-4 pt-3 pb-safe bg-rh-bg-primary border-t border-rh-border-subtle'>
          <button
            type='button'
          ...
          >
            ...
            {!isSubmitting && <CircleCheck size={18} strokeWidth={2.5} />}
          </button>
        </div>
      </div>
      ...
      <AnimatePresence>
        {guideMessage && (
          ...
          >
            <span className='text-sm text-white'>{guideMessage}</span>
          </motion.div>
        )}
      </AnimatePresence>
...
const ConfirmModal = memo(function ConfirmModal({
  ...
  const remaining = selectedUsers.length - previewCount;
    ...
    <>
      ...
      >
        ...
        <div className='flex items-center justify-between'>
          <h3 className='text-lg font-semibold text-white'>일괄 등록 확인</h3>
          <button type='button' onClick={onCancel}>
            <X size={22} className='text-rh-text-secondary' />
          </button>
        </div>

        {/* 요약 */}
        <div className='flex items-center gap-3'>
          <Users size={28} className='text-rh-accent shrink-0' />
          <div>
            <p className='text-[15px] font-semibold text-white'>
              {selectedCount}명의 출석을 등록합니다
            </p>
            <p className='text-xs text-rh-text-secondary'>
              전체 {totalCount}명 중 {selectedCount}명 선택됨
            </p>
          </div>
        </div>
        ...
        <div className='flex flex-wrap gap-1.5'>
          <InfoChip icon={Calendar} label={dateLabel} active />
          <InfoChip icon={Clock} label={timeLabel} active />
          <InfoChip icon={MapPin} label={locationLabel} active />
          <InfoChip icon={Zap} label={exerciseLabel} active />
        </div>
        ...
        <div className='space-y-2'>
          ...
          {previewUsers.map((u) => (
            <div key={u.id} className='flex items-center gap-2'>
              <CircleCheck size={16} className='text-rh-accent shrink-0' />
              <span className='text-[13px] font-medium text-white'>
                {u.first_name}
                {u.birth_year && (
                  <span className='text-rh-text-secondary'>
                    ({getBirthSuffix(u.birth_year)})
                  </span>
                )}
              </span>
              {hostIds.has(u.id) && (
