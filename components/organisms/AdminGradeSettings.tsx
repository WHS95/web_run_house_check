...
export default function AdminGradeSettings({
  ...
  const [addFormData, setAddFormData] =
    useState<GradeFormData>(defaultFormData);
  ...
  const fetchGrades = useCallback(async () => {
    try {
      ...
      if (result.success) {
        setGrades((result.data ?? []) as unknown as CrewGrade[]);
      }
    } catch {
      haptic.error();
    } finally {
  ...
  const handleEditGrade = useCallback(
    async (gradeId: number) => {
      ...
      try {
        ...
        haptic.error();
        showNotification("등급 수정 중 오류가 발생했습니다.", "error");
      } finally {
        setActionLoading(false);
      }
    },
        ...
        onConfirm: async () => {
          try {
            ...
            if (result.success) {
              ...
              setOpenMenuId(null);
              await fetchGrades();
              showNotification("등급이 삭제되었습니다.", "success");
            } else {
              haptic.error();
              showNotification(
            ...
            haptic.error();
            showNotification("등급 삭제 중 오류가 발생했습니다.", "error");
          } finally {
            setActionLoading(false);
          }
        },
  ...
  const renderForm = (
    ...
    <div className='space-y-3'>
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setFormData({
                ...formData,
                min_attendance_count: parseInt(e.target.value) || 0,
              })
            }
            className='w-full bg-rh-bg-primary border border-rh-border rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-rh-accent'
          />
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setFormData({
                ...formData,
                min_hosting_count: parseInt(e.target.value) || 0,
              })
            }
            className='w-full bg-rh-bg-primary border border-rh-border rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-rh-accent'
          />
      ...
      <div className='flex items-center justify-between'>
        ...
        <button
          ...
          className={`relative w-11 h-6 rounded-full transition-colors ${
            formData.can_host ? "bg-rh-accent" : "bg-rh-bg-muted"
          }`}
        >
          <span
            className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full transition-transform ${
              formData.can_host ? "translate-x-5" : "translate-x-0"
            }`}
          />
        </button>
      </div>
      ...
      <div className='flex gap-2 pt-1'>
        <button
          onClick={onSave}
          disabled={actionLoading || !formData.name_override.trim()}
    ...
    return (
      <div className='space-y-3'>
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className='bg-rh-bg-surface rounded-xl p-4 h-24' />
        ))}
      </div>
    );
  }
