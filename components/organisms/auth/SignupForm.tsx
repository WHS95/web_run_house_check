...
const SignupForm: React.FC = () => {
  ...
  const inputStyle =
    "w-full px-3 py-2 border border-rh-border rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm";
  const buttonStyle =
    "w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:bg-rh-bg-muted";
  const errorTextStyle = "mt-1 text-xs text-rh-status-error";
      ...
      <div className='px-4 py-8 bg-white shadow sm:rounded-lg sm:px-10'>
        <form onSubmit={handleSubmit(onSubmit)} className='space-y-6'>
          <div>
            <label
              htmlFor='firstName'
              className='block text-sm font-medium text-rh-text-muted'
            >
              이름
            </label>
            <input
