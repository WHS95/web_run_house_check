...
export default function AdminGradeAssignment({
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (gradesResult.success) {
        const gradesList: CrewGrade[] = (gradesResult.data ??
          []) as unknown as CrewGrade[];
        gradesList.sort((a, b) => a.sort_order - b.sort_order);
        setGrades(gradesList);
      }
    } catch (error) {
      console.error("등급 할당 데이터 로딩 실패:", error);
    } finally {
      setLoading(false);
    }
  }, [crewId]);
  ...
  const handleGradeAssign = async (userId: string, gradeId: number) => {
    triggerHaptic();
  ...
  const getMemberGrade = (member: Member): CrewGrade | undefined => {
    if (!member.crew_grade_id) return undefined;
    return grades.find((g) => g.id === member.crew_grade_id);
  };
    ...
    <div className='space-y-3'>
      ...
      <div className='space-y-2'>
        {filteredMembers.map((member, index) => {
          const memberGrade = getMemberGrade(member);
          ...
          return (
            ...
            >
              ...
              <div className='flex flex-col items-end gap-1 shrink-0 ml-2'>
                ...
                <div className='relative'>
                  <select
                    value={member.crew_grade_id ?? ""}
                    onChange={(e) => {
                      const gradeId = Number(e.target.value);
                      if (gradeId) {
                        handleGradeAssign(member.id, gradeId);
                      }
                    }}
                ...
                {member.grade_override && (
                  <button
                    onClick={() => handleResetOverride(member.id)}
                    disabled={isAssigning}
