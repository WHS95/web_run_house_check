...
const PlaceBarChart = memo(function PlaceBarChart({
  ...
  return (
    <div className={"bg-rh-bg-surface rounded-xl" + " p-4 space-y-3"}>
      <div className={"flex items-center" + " justify-between"}>
        ...
        <Link
          href={
            "/admin2/analyze/place-detail" + `?year=${year}` + `&month=${month}`
          }
          className={"text-xs" + " text-rh-text-tertiary"}
        >
          상세
        </Link>
      </div>
      <div className='space-y-2.5'>
        {data.length > 0 ? (
          data.map((item, idx) => (
            <div key={item.locationName} className='space-y-1'>
              ...
              <AdminProgressBar
                percent={item.participationRate}
                fillColor={FILL[idx] ?? "bg-rh-accent"}
              />
            </div>
          ))
        ) : (
          <p
            className={
              "py-4 text-center" + " text-sm" + " text-rh-text-secondary"
            }
          >
            데이터가 없습니다.
          </p>
