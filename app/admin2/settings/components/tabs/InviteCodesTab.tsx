...
import { AdminBadge, AdminAlertDialog } from "@/app/admin2/components/ui";
...
const InviteCodesTab = memo(function InviteCodesTab({
  ...
  const [regenerateTarget, setRegenerateTarget] = useState<InviteCode | null>(
    null,
  );
  ...
  return (
    <div className='space-y-4'>
      ...
      <div className='flex justify-between items-center'>
        <h3 className='text-sm font-semibold text-white'>초대코드 목록</h3>
        <span className='text-xs text-rh-text-secondary'>{codes.length}개</span>
      </div>
              ...
              <div className='px-4 py-4 space-y-3 rounded-xl bg-rh-bg-surface'>
                <div className='flex justify-between items-center'>
                  <div className='flex gap-2 items-center'>
                    ...
                    <AdminBadge variant='accent'>활성</AdminBadge>
                  </div>
                  <div className='flex gap-1 items-center'>
                    <button
                      onClick={() => handleCopy(code.invite_code)}
                ...
                <div className='flex gap-4 text-xs text-rh-text-secondary'>
                  {code.use_count !== undefined && (
                    <span>사용 횟수: {code.use_count}회</span>
                  )}
        ...
        <div className='px-4 py-4 space-y-3 rounded-xl border bg-rh-bg-surface border-rh-accent'>
          ...
          <div className='flex gap-2'>
            ...
            <button
              ...
              disabled={saving || newCode.length !== 7}
