    ...
    <div className='h-[76px] min-w-0 flex flex-col items-center justify-center gap-1 rounded-2xl bg-rh-bg-surface py-4 px-2'>
      <span
        className={
          ...
          (valueSize === "lg" ? "text-xl" : "text-[13px]")
        }
      >
        {value}
      </span>
...
export default function UserDetail({ detail, crewId }: Props) {
  ...
  const handleToggle = useCallback(async () => {
    ...
    const prev = active;
    setActive(next);
    const { error } = await updateUserStatus(user.id, crewId, next);
    if (error) {
      setActive(prev);
      alert("상태 변경에 실패했습니다.");
    }
    ...
    <FadeIn>
      <div className='flex-1 px-4 pt-4 pb-4 flex flex-col gap-4'>
        ...
        <div className='rounded-2xl bg-rh-bg-surface p-6 flex flex-col items-center gap-4'>
          ...
          {isStaff && <AdminBadge variant='accent'>운영진</AdminBadge>}
        ...
        <div className='grid grid-cols-3 gap-2'>
          <StatCard
            value={formatDate(detail.last_attendance_date)}
            label='최근 참여일'
            valueSize='sm'
          />
          <StatCard value={`${detail.attendance_count}회`} label='전체 출석' />
          <StatCard value={`${detail.hosted_count}회`} label='모임 개설' />
        </div>

        {/* 회원 관리 */}
        <section className='flex flex-col gap-3'>
          <h2 className='text-base font-semibold text-white px-1'>회원 관리</h2>
