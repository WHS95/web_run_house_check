...
export default function AdminGradeRecommendations({
  ...
  const fetchRecommendations = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        setRecommendations((result.data ?? []) as unknown as Recommendation[]);
      }
    } catch (error) {
      console.error("등급 추천 데이터를 불러오는 중 오류 발생:", error);
    } finally {
      setLoading(false);
    }
  }, [crewId]);
  ...
  const handleApproveAll = useCallback(async () => {
    ...
    try {
      const result = await approveAllGradeRecommendationsAction({
        crewId,
      });
      if (result.success) {
        haptic.success();
  ...
  if (recommendations.length === 0) {
    return (
      <div className='flex flex-col items-center justify-center py-16'>
        ...
        <p className='text-rh-text-tertiary text-sm'>변경 추천이 없습니다</p>
      </div>
    );
  }
  ...
  return (
    <div className='space-y-4'>
      ...
      <div className='flex items-center justify-between'>
        <div className='flex items-center gap-2'>
          <h3 className='text-white text-base font-semibold'>등급 변경 추천</h3>
      ...
      <div className='space-y-3'>
        {recommendations.map((rec) => {
          ...
          return (
            <div key={rec.user_id} className='bg-rh-bg-surface rounded-xl p-4'>
              {/* 사용자 이름 및 등급 변경 */}
              <div className='flex items-center justify-between mb-3'>
              ...
              <div className='flex gap-2'>
                <button
                  onClick={() => handleDeny(rec.user_id)}
                ...
                <button
                  onClick={() =>
                    handleApprove(rec.user_id, rec.recommended_grade_id)
                  }
                  disabled={isProcessing}
